...
export const getAdminMetrics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [
      ...
      supabaseAdmin.from("profiles").select("user_id, trust_score, verified, created_at"),
      supabaseAdmin.from("funding_requests").select("id, amount_requested, currency, status, human_approval, sector, region, created_at"),
      supabaseAdmin.from("verification_events").select("id, status, kind, created_at"),
      supabaseAdmin.from("risk_scores").select("trust_score, risk_level, computed_at").order("computed_at", { ascending: false }).limit(500),
      supabaseAdmin.from("profiles").select("user_id, created_at").gte("created_at", thirtyDaysAgo),
    ]);
    ...
    // Risk level distribution from latest risk_scores
    const riskDist: Record<string, number> = {};
    const seen = new Set<string>();
    for (const r of (riskScores ?? [])) {
      // riskScores ordered desc, so first occurrence per user is latest
    ...
    return {
      ...
      trustDistribution: Object.entries(trustBuckets).map(([range, count]) => ({ range, count })),
